...
function res = grad(model, data, wd_coefficient)
  ...
  % The returned object is supposed to be exactly like parameter <model>, i.e. it has fields res.input_to_hid and res.hid_to_class. 
  ...
  dEc_dzo = (1/m)*(class_prob-data.targets);
  dEc_dwho = dEc_dzo*hid_output';
  dEc_dwih = (dEc_dzo'*model.hid_to_class)'.*(hid_output.*(1-hid_output))*data.inputs';
  dEw_dwho = wd_coefficient.*model.hid_to_class;
  dEw_dwih = wd_coefficient.*model.input_to_hid;
  
  res.input_to_hid = dEw_dwih + dEc_dwih;
  res.hid_to_class = dEw_dwho + dEc_dwho;
...
function test_gradient(model, data, wd_coefficient)
  ...
  hid_to_class_theta_size = numel(model.hid_to_class);
